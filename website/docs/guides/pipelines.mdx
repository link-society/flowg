...
import NodeAlertZapierUrl from '@site/static/img/guides/pipelines/node-alert-zapier.png'

Let's assume we have configured an alert named ``zapier`` with the following
webhook URL:
...
>
  ...
  >
    ...
    <div>
      <img src={NodeAlertZapierUrl} alt="Alert node" />
    </div>
  </div>
