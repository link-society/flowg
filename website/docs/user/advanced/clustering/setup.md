...
It is recommended to only use DNS management script on custom DNS servers where DNS propagation won't be an issue.

You will need to create a script that accepts 3 arguments:
```shell
./dns-script {set,del} [dns record type] [dns record]
```
