...
<ForwarderForm
  forwarderType="Syslog Server"
  fields={[
    { label: 'Network', value: 'TCP', tooltip: 'UDP works as well; Logstash starts a listener for each protocol.' },
    { label: 'Server Address', value: '127.0.0.1:50000', tooltip: 'Assuming your FlowG is running on the same host as the ELK stack.' },
    { type: 'dynamic', label: 'Tag', value: 'flowg', tooltip: 'You can use any static value or expression.' },
    { type: 'dynamic', label: 'Severity', value: 'INFO', tooltip: 'You can use any static value or expression.' },
    { type: 'dynamic', label: 'Facility', value: 'LOCAL0', tooltip: 'You can use any static value or expression.' },
    { type: 'dynamic', label: 'Message', value: '@expr:toJSON(log)', tooltip: 'The output must be a JSON because of how we configured Logstash.' },
  ]}
/>
...
<PipelineDiagram
  alt="A FlowG pipeline routing syslog logs to the ELK forwarder"
  nodes={[
    { id: 'source', type: 'source', x: 0, y: 0, value: 'SYSLOG' },
    { id: 'fwd', type: 'forwarder', x: 340, y: 0, value: 'elk' },
  ]}
  edges={[
    { from: 'source', to: 'fwd' },
  ]}
