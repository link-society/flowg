...
The package at `internal/storage/backends/badger/concrete/config/transactions`
holds the generic read/write operations behind the config backend. Each function
takes a Badger transaction and an `itemType` selecting the namespace to operate
on; this document describes how that key space is laid out.
