...
import { InputListRoot, InputListRow } from './styles'
import { InputListProps, Row } from './types'

const genId = () => `id_${Date.now()}_${Math.random().toString(36).slice(2)}` // NOSONAR: non-security React key
...
const InputList = (props: InputListProps) => {
  ...
  return (
    <InputListRoot id={props.id ?? 'field:generic.list-editor'}>
      {rows.map((row, index) => (
        <InputListRow
