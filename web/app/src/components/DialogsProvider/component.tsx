...
const DialogsProvider = (props: DialogsProviderProps) => {
  ...
  const removeFromStack = (dialog: Promise<any>) => {
  ...
  const closeDialogUi = useEventCallback(<R,>(dialog: Promise<R>) => {
    ...
    setTimeout(() => removeFromStack(dialog), unmountAfter)
  })
  ...
  const renderDialogEntry = ({
    key,
    open,
    Component,
    payload,
    promise,
  }: DialogStackEntry<any, any>) => {
