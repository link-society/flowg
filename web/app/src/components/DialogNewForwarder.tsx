...
const DialogNewForwarder = ({
  ...
  const [type, setType] = useState<ForwarderConfigTypes>('http')

  const [onSubmit, loading] = useApiOperation(async () => {
    await configApi.saveForwarder(name, {
      config: ForwarderConfigFactory(type),
    })
    onClose(name)
  }, [name, type])
        ...
        <div className="pt-3 w-full flex flex-col items-stretch gap-3">
          ...
          <FormControl fullWidth>
            ...
            <Select<ForwarderConfigTypes>
              ...
              onChange={(e) => setType(e.target.value as ForwarderConfigTypes)}
            >
